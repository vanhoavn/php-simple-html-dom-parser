 ...
 *
 * @property string $outertext Full html code of this node include its own tag
 * @property string $innertext Full html code of this node exclude its own tag
 * @property string $plaintext Full text this node
 */
class SimpleHtmlDomNode
{
     ...
     * @return SimpleHtmlDomNode|null
     */
    function find_nth($selector, $idx, $lowercase = false)
    {
        $found = $this->find($selector, $lowercase);
    ...
    protected function parse_selector($selector_string)
    {
        ...
        //        $pattern = "/([-0-9a-zA-Z:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        $pattern = "/([-0-9a-zA-Z:\*]*)(?:\#([-0-9a-zA-Z]+)|\.([-0-9a-zA-Z]+))?(?:\[@?(!?[-0-9a-zA-Z:]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
     ...
     * @return array|false an array containing the 'height' and 'width' of the image on the page or false if we can't
     *                     figure it out.
     */
    function get_display_size()
    {
        ...
        if (isset($this->attr['style'])) {
            ...
            $attributes = [];
            preg_match_all("/([-0-9a-zA-Z]+)\s*:\s*([^;]+)\s*;?/", $this->attr['style'], $matches, PREG_SET_ORDER);
